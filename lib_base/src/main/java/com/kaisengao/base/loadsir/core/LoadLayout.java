package com.kaisengao.base.loadsir.core;
...
import com.kaisengao.base.loadsir.callback.Callback;
import com.kaisengao.base.loadsir.callback.SuccessCallback;
import com.kaisengao.base.loadsir.LoadSirUtil;
...
public class LoadLayout extends FrameLayout {
        ...
        for (Class key : callbacks.keySet()) {
            if (key == status) {
                ...
                if (key == SuccessCallback.class) {
                    ...
                    addView(rootView);
                    callbacks.get(key).onAttach(mContext, rootView);
                }
                preCallback = status;
            }
        }
    ...
    public void setCallBack(Class<? extends Callback> callback, Transport transport) {
        ...
        checkCallbackExist(callback);
        transport.order(mContext, callbacks.get(callback).obtainRootView());
    }
